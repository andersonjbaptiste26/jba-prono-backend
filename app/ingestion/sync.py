    ...
    for game in games:
        ...
        if not match:
            match = Match(
                ...
                kickoff_at=kickoff, status="scheduled", external_id=game["id"],
            )
            db.add(match)
            db.flush()
            created += 1
...
def _upsert_event(db: Session, match: Match, event_type: str, label: str, odds_value: float, market_key: str, selection: str, bookmaker_key: str):
    db.add(Odds(match_id=match.id, market=market_key, selection=selection, value=odds_value, source=bookmaker_key))
...
def _sync_totals_events(db: Session, match: Match, bookmaker: dict):
    ...
    for outcome in totals_market["outcomes"]:
        ...
        if point is None:
            continue
        label = f"+{point} buts" if outcome["name"] == "Over" else f"-{point} buts"
...
def sync_all_leagues(db: Session) -> list[dict]:
    ...
    for sport_key, league_name in LEAGUE_KEYS.items():
        try:
            results.append(sync_league(db, sport_key, league_name))
